...
|--------------------------|--------------------------------------------------------------------------------------|------------------------------------------------------------------------|
| USD / Stable‑coin (USDC) | Pay invoice or swipe card → Dria auto‑converts at the daily TWAP, minus ~1–2% FX fee | $DRIA credits appear in your dashboard, locked for compute spend       |
| Direct $DRIA             | Send tokens straight to your project wallet; no fee                                  | Credits top‑up instantly, 1:1                                          |

**Credits never leave the system:** they can only be burnt for FLOPS.
...
sequenceDiagram
    participant User
    participant API as Dria /batch
    participant Scheduler
    participant Workers as GPU • NPU • CPU Swarm
    participant Solana
    
    User->>API: POST /batch (model, data, QoS)
    ...
    API-->>User: JSON (output, cost, proof_link)
```
...
4. **Serve /train /batch** – Hit the same REST endpoints your devs use against the public network.
...
|------|---------|--------------|
| You buy $10k USD → credits | +10k | none |
| You run jobs on‑prem | −8k | none |
| You export 200 GPU‑hr idle | +2k (burn) | −2k supply |

Result: zero net spend, deflationary pressure on the token, and 100% utilisation of your silicon.
...
curl -X POST https://api.dria.ai/batch \
     -H "Authorization: Bearer $DRIA_KEY" \
