...
When a user toggles **Verifiable Inference**, the Dria compiler instruments each shard to output a tiny polynomial commitment of its top‑k activations (see TOPLOC).
Nodes return the result **plus** the proof; Solana smart‑contracts verify in less than 1 second.
*Proof passes → reward unlocked.  Proof fails or times‑out → stake slashed and job re‑queued.*
Users gain cryptographic trust without zk‑ML overhead.
